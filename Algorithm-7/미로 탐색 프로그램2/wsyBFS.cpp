...
//	locQueue.enqueue(n);	// 덱에 입구 위치 삽입
//
//	while(locQueue.isEmpty() == false) {	// 뎅이 비어있지 않는 동안
//		n = locQueue.dequeue();
//		Location2D here(n->row, n->col);	// 덱의 front 상단 객체 복사
//		delete n;
//		//locQueue.pop();	// 덱 상단 객체 삭제
//
//		int r = here.row;
//		int c = here.col;
...
//			if(isValidLoc(r - 1, c))
//				locQueue.enqueue(new Node(Location2D(r - 1, c)));
//			if(isValidLoc(r + 1, c))
//				locQueue.enqueue(new Node(Location2D(r + 1, c)));
//			if(isValidLoc(r, c - 1))
//				locQueue.enqueue(new Node(Location2D(r, c - 1)));
//			if(isValidLoc(r, c + 1))
//				locQueue.enqueue(new Node(Location2D(r, c + 1)));
//		}
//	}
